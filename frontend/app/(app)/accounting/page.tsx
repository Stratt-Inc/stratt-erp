...
const typeColors: Record<string, string> = {
  asset: "hsl(var(--primary))", liability: "hsl(var(--destructive))", equity: "hsl(var(--accent))",
  revenue: "hsl(var(--accent))", expense: "hsl(var(--warning))",
};
                ...
                <tr key={a.id} className="data-row">
                  ...
                  <td className="px-4 py-2">
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full"
                      style={{ background: typeColors[a.type] ? `color-mix(in srgb, ${typeColors[a.type]} 8%, transparent)` : "rgba(107,114,128,0.08)", color: typeColors[a.type] ?? "#6B7280" }}>
                      {typeLabels[a.type] ?? a.type}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-right text-sm num font-bold"
                    style={{ color: a.balance >= 0 ? "hsl(var(--accent))" : "hsl(var(--destructive))" }}>
                ...
                <tr key={t.id} className="data-row">
                  ...
                  <td className="px-4 py-2">
                    <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full"
                      style={{ background: t.type === "credit" ? "hsl(var(--accent) / 0.1)" : "hsl(var(--destructive) / 0.1)", color: t.type === "credit" ? "hsl(var(--accent))" : "hsl(var(--destructive))" }}>
                      {t.type === "credit" ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                      {t.type === "credit" ? "Crédit" : "Débit"}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-right text-sm num font-bold"
                    style={{ color: t.type === "credit" ? "hsl(var(--accent))" : "hsl(var(--destructive))" }}>
